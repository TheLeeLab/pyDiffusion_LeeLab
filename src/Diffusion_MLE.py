...
class D_MLE:
    ...
    def DSigma_MLE_BootStrap(
        ...
        for i in np.arange(n_samples):
            diff_coordinates = np.random.choice(
            ...
            dX = np.subtract(
                diff_coordinates,
                np.tile(np.mean(diff_coordinates, axis=0), (len(coordinates) - 1, 1)),
            )

            sine_transform = np.square(dst(dX, 1, axis=0) / 2.0)
            optimfunc = lambda x: -self.likelihood_subfunction(
                sine_transform, x[0], x[1], dT, R, n_d
            )
    ...
    def DSigma_MLE(
        ...
        dX = np.subtract(
            ...
            np.tile(
                np.mean(np.diff(coordinates, axis=0), axis=0), (len(coordinates) - 1, 1)
            ),
        )
