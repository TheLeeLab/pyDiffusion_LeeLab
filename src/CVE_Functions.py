...
class CVE:
    ...
    def DSigma_CVE_BootStrap(
        self, coordinates, dT, R=1.0 / 6, n_d=1, min_points=10, n_samples=1000
    ):
        """
        ...
        if n_d > 1:
            try:
                if coordinates.shape[1] != n_d:
                    raise Exception(
                        "Dimension of coordinates and n_d are inconsistent."
                    )
        ...
        try:
            if coordinates.shape[0] < min_points:
                raise Exception(
                    """Not enough points to calculate a reliable estimate of diffusion coefficient.
        ...
        for k in np.arange(n_samples):
            displacements = np.random.choice(
                samples, size=(coordinates.shape[0] - 1, n_d)
            )
            new_coordinates = np.vstack([r0, np.cumsum(displacements, axis=0)])
            for i in np.arange(n_d):
                D_d_i[i, k], sigma, var_d_i[i, k] = self.Eq14(
                    new_coordinates[:, i], dT, R
                )
    ...
    def DSigma_CVE(self, coordinates, dT, R=1.0 / 6, n_d=1, min_points=10):
        """
    ...
    def Eq14(self, x, dT, R=1.0 / 6, min_points=10):
        """CVE_Eq14 function
        ...
        epsilon = np.subtract(
            np.divide(np.square(sigma), np.multiply(D, dT)), np.multiply(2, R)
        )
        N = len(x)
        varD = np.multiply(
            np.square(D),
            (
                ((6 + 4 * epsilon + 2 * np.square(epsilon)) / N)
                + ((4 * np.square(1 + epsilon)) / np.square(N))
            ),
        )
        return D, sigma, varD

    def Eq16(self, x, sigma, dT, R=1.0 / 6, min_points=10):
        """CVE_Eq16 function
        ...
        deltaX_sqr = np.mean(np.square(diffX))
        D = np.divide(
            np.subtract(deltaX_sqr, np.multiply(2.0, sigma_squared)),
            np.multiply(np.multiply(2, np.subtract(1.0, np.multiply(2.0, R))), dT),
        )
        ...
        varD = np.add(
            np.divide(
                np.multiply(np.square(D), (2 + 4 * epsilon + 3 * np.square(epsilon))),
                N * np.square(1 - 2 * R),
            ),
    ...
    def Eq10(self, D, deltaT, sigma, k, N, R=1.0 / 6):
        """Eq10 function
        ...
        term2_2 = np.multiply(2.0, np.multiply(D, np.multiply(R, np.square(deltaT))))
        term3 = np.subtract(
            1.0, np.cos(np.divide(np.multiply(np.pi, k), np.add(N, 1.0)))
        )
        term2 = np.multiply(np.multiply(2.0, np.subtract(term2_1, term2_2)), term3)
